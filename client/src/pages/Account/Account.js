...
export default function Account() {
    ...
    async function getUserInfo() {
        setLoadingContent(true);
        try {
            const { data } = await axios.get("/user/account-content");
            setUserData(data.user);
            setVideos(data.user.videos.reverse());
            setUnlockedMedia(data.user.unlocked.reverse());
            setLikedMedia(data.user.likes.reverse());
    ...
    function getTopSupporters(transactions) {
        ...
        const depositFilteredArray = transactions.filter(tx => tx.fromAddress != null)
        const userFilteredArray = depositFilteredArray.filter(tx => tx.fromAddress.toUpperCase() != user.address.toUpperCase());


        // loop through each user object and aggregate their payment amount
        userFilteredArray.forEach(tx => {
        ...
        const sortedUsers = Array.from(userMap, ([address, amount]) => {
            const { from } = userFilteredArray.find(tx => tx.fromAddress.toUpperCase() === address.toUpperCase());
    ...
    return (
        <div>
            <HeaderMain />
            {settingsOpen ? <SettingsModal user={user} saveChanges={() => getUserInfo()} close={() => setSettingsOpen(false)} /> : null}
            <div id="walletFlex">
                <div id="walletArea">
                    <div id="userHeroArea">
                        {/* <div id="mEthEarned">
                            ${user?.solEarned.toFixed(2)} earned
                        </div>
                        <div id="mLastActive">
                            0 collectors
                        </div> */}
                        ...
                        <div id="walletNameCollect">
                            ...
                            <div>
                                <div id="sendCollectBtns">
                                    ...
                                    <div id="settingsBtn" onClick={() => setSettingsOpen(true)}>
                                        settings
                                    </div>
                                </div>
                            </div>
