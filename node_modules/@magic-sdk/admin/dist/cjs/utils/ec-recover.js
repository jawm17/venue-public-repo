...
function publicKeyToAddress(publicKey) {
    var address = keccak_js_1.keccak256(publicKey.slice(1)).slice(-20);
    return "0x" + utils_js_1.bytesToHex(address);
}
/**
...
function ecRecover(data, signature) {
    ...
    var msg = utils_js_1.utf8ToBytes(data);
    var sig = utils_js_1.hexToBytes(prepareSignature(signature));
    var recovery = getRecoveryBit(sig);
    var hash = hashPersonalMessage(msg);
    var publicKey = secp256k1_compat_js_1.ecdsaRecover(sig.slice(0, 64), recovery, hash, false);
    var assertPublicKey = secp256k1_compat_js_1.publicKeyConvert(publicKey, false);
